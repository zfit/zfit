...
def test_padreflect_data_weights_1dim(weights):
    ...
    true_data = np.sort(np.array([-2.8, -2, 0.1, 0.5, -2, 5, 6.7, -1.2, 1.2, 7, 8.0, 8.3]))
    np.testing.assert_allclose(true_data, np.sort(data))
    if weights is not None:
    ...
    true_data = np.sort(np.array([-4.5, -4.1, -2.8, -2, 0.1, 0.5, -2, 5, 6.7, -1.2, 1.2, 7, 8.0, 8.3, 10]))
    np.testing.assert_allclose(true_data, np.sort(data))
    if weights is not None:
        true_w = np.sort(np.array([1, 2, 3, 6, 1, 2, 3, 4, 5, 6, 7, 8, 4, 5, 8]))

    data, w = padreflect_data_weights_1dim(testarray, mode={'lowermirror': 0.3}, limits=testobs.limits)
    true_data = np.sort(np.array([-4.5, -4.1, -2.8, -2, 0.1, 0.5, -2, 5, 6.7, -1.2, 1.2, 7]))
    np.testing.assert_allclose(true_data, np.sort(data))
    if weights is not None:
        true_w = np.sort(np.array([1, 2, 3, 6, 1, 2, 3, 4, 5, 6, 7, 8]))

    data, w = padreflect_data_weights_1dim(testarray, mode={'uppermirror': 0.3}, limits=testobs.limits)
