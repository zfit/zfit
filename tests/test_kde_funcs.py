...
def test_padreflect_data_weights_1dim(weights):
    ...
    data, w = padreflect_data_weights_1dim(testarray, mode={'lower': 0.1, 'upper': 0.1}, weights=weights)
    true_data = np.array([-2.8, -2, -2, -1.2, 0.1, 0.5, 1.2, 5, 6.7, 7, 7, 7.3])
    np.testing.assert_allclose(true_data, np.sort(data))
    if weights is not None:
        true_w = np.sort(np.array([6, 3, 1., 2., 3., 4., 5., 6., 7., 8., 8, 5]))
    ...
    data, w = padreflect_data_weights_1dim(testarray, mode={'lower': 0.1, 'upper': 0.3}, weights=weights)
    ...
    data, w = padreflect_data_weights_1dim(testarray, mode={'lower': 0.1}, weights=weights)
    ...
    data, w = padreflect_data_weights_1dim(testarray, mode={'lower': 0.3}, weights=weights)
    ...
    data, w = padreflect_data_weights_1dim(testarray, mode={'upper': 0.3}, weights=weights)
    ...
    with pytest.raises(ValueError):
        data, w = padreflect_data_weights_1dim(testarray, mode={'upper': 0.3, 'lowe': 0.321}, weights=weights)
