...
def test_pdf_cache_is_used():
    ...
    x = znp.linspace(-5, 5, 500)
    cached_gauss.pdf(x)
    assert tf.equal(cached_gauss.pdf_cache_counter, tf.Variable(0.0))
    cached_gauss.pdf(x)
    assert tf.equal(cached_gauss.pdf_cache_counter, tf.Variable(1.0))
...
def test_integrate_cache_is_used():
    ...
    assert tf.equal(cached_gauss.integrate_cache_counter, tf.Variable(0.0))
    cached_gauss.integrate(limits=obs)
    assert tf.equal(cached_gauss.integrate_cache_counter, tf.Variable(1.0))
