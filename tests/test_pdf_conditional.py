...
def test_conditional_pdf_simple():
    ...
    for i, (xval, muval) in enumerate(data2d.value()):
        mu.assign(muval)
        prob_check2[i] = float(np.asarray(gauss.pdf(xval)).reshape(-1)[0])
    ...
    for i, xval in enumerate(data1d.value()[:, 0]):
        mu.assign(xval)
        prob_check1[i] = float(np.asarray(gauss.pdf(xval)).reshape(-1)[0])
    ...
    for i, (xval, muval, sigmaval) in enumerate(data3d.value()):
        ...
        prob_check3[i] = float(np.asarray(gauss.pdf(xval)).reshape(-1)[0])
