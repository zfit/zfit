...
def HelicityAmplitude(x, spin) :
  ...
  """
  if spin == 0 : return tf.complex( Const(1.), Const(0.))
  if spin == 1 : return tf.complex( x, Const(0.))
  if spin == 2 : return tf.complex( (3.*x**2-1.)/2., Const(0.))
  if spin == 3 : return tf.complex( (5.*x**3-3.*x)/2., Const(0.))
  if spin == 4 : return tf.complex( (35.*x**4-30.*x**2+3.)/8., Const(0.))
  return None
...
def ResonantLASSLineShape(m2ab, m0, gamma0, a, r, ma, mb) :
  ...
  ampl = RelativisticBreitWigner(m2ab, m0, width)*tf.complex(tf.cos(2.*phase), tf.sin(2.*phase))*CastComplex(m0*m0*gamma0/q0)
  return ampl
