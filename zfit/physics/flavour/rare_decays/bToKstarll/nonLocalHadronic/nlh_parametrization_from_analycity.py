...
t_H_zero = 4.0 * tf.square(const.MD) - tf.sqrt(4.0 * tf.square(const.MD)) * tf.sqrt(4.0 * tf.square(const.MD) - tf.square(const.Mpsi2S))
...
def poly3C(coeff0, coeff1, coeff2, coeff3, x): # take x as real and then convert as complex
  return coeff0 + coeff1 * CastComplex(x) + coeff2 * CastComplex(tf.square(x)) + coeff3 * CastComplex(tf.pow(x,3))

def poly4C(coeff0, coeff1, coeff2, coeff3, coeff4, x): # take x as real and then convert as complex
  return coeff0 + coeff1 * CastComplex(x) + coeff2 * CastComplex(tf.square(x)) + coeff3 * CastComplex(tf.pow(x,3)) + coeff4 * CastComplex(tf.pow(x,4))
...
def zz(t, t_plus, t_zero): # t is complex
  zeta = (tf.sqrt(CastComplex(t_plus) - t) - tf.sqrt(CastComplex(t_plus) - CastComplex(t_zero))) / (tf.sqrt(CastComplex(t_plus) - t) + tf.sqrt(CastComplex(t_plus) - CastComplex(t_zero)))
  return zeta
