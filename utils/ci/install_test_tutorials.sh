...
source "${BASEDIR}/.test_tutorials_env/bin/activate" && pip install -U pip uv && uv pip install "${BASEDIR}/../../[all]" && uv pip install nbval
...
uv pip install -r zfit-tutorials/requirements.txt 2>&1 | tail -n 11 &&
