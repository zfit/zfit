...
- Add ``ClipPDF`` functor and ``create_clipped()`` method to ``BasePDF`` for handling PDFs that can produce negative or NaN values (e.g., KDE with negative weights). The clipping operation uses ``znp.maximum`` to ensure output values are at least the specified lower bound (default 1e-310).
